...
LowPass::LowPass(int order, double cutoffFrequency, double sampleRate) : Filter(order), prev_output(0.0f) {
    m_coefficients.resize(m_order + 1);
    /* Normalize the frequency via cutoffFrequency / Nyquist Frequency */
    double norm_cutoff = cutoffFrequency / (sampleRate / 2.0);
    ...
    for (int n = 0; n <= m_order; ++n) {
        ...
        if (n == m_order / 2) {
            ...
            m_coefficients[n] = sin(2 * M_PI * norm_cutoff * (n - m_order / 2)) /
                                 (M_PI * (n - m_order / 2));
...
void LowPass::process(const std::vector<double>& input, std::vector<double>& output) {
    ...
    for (size_t n = 0; n < input.size(); ++n) {
        ...
        for (int k = 0; k <= m_order; ++k) {
            if (n >= k) {
                output[n] += m_coefficients[k] * input[n - k];
            }
        }
    }
}
